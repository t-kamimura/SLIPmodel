...
def eventHandler(z, phase, x0):
    if phase == 0:
        ...
        if toe < 0 and z[3] < 0: # 接地判定(テンプレから，andで条件を追加)
            print("touch down")
            x0 = z[0] + z[1] * np.tan(gamma) # 接地点の更新(ここもエネルギー誤差を抑えるうえで重要)
            # x0 = z[0] + z[1] * np.sin(gamma) # 接地点の更新
            phase = 1
    elif phase == 1:
        # stance phase
        l = np.sqrt((z[0] - x0)**2 + z[1]**2)
        extension = l - l0 # 脚ばねの伸び．負の値なら圧縮
        if extension > 0 and z[3] > 0: # 離地判定
            print("lift off")
            phase = 0
...
def RK4(z0, dt, tend):
    ...
    pout = np.copy(phase)
    Eout = np.array([calc_energy(z, phase, x0)])
    ...
    while t < tend:
        ...
        Eout = np.vstack((Eout, calc_energy(z, phase, x0))) # エネルギーを積み上げる
...
print("Energy error: ", Eout[0] - Eout[-1])
...
def make_animation(tout, zout, toeout):
    ...
    fps = 30
    skip = round(1 / (dt * fps))
    ani = animation.FuncAnimation(fig, update, frames=range(0, len(tout), skip),
                                  init_func=init, blit=True, interval=dt*skip*1000)
